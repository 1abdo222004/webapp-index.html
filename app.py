from flask import Flask, request, render_template, jsonify from flask_cors import CORS import requests import urllib.parse as ul import json
...
@app.route('/id', methods=['POST']) def receive_data(): content = request.json init_data = content.get("initData")
...
@app.route('/idinfo') def id_info(): user_id = request.args.get("id") if user_id in data_store: return jsonify(data_store[user_id]) return "لم يتم العثور على بيانات لهذا المستخدم.", 404
